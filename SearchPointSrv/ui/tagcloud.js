...
export function createWc(words, centerX, centerY, scale, stages) {
  var n = words.length;
  var placed = [];
  ...
  for (var i = 0; i < n; i++) {
    ...
    word.width = getWordW(word, stages);
...
export function getWordH(word) {
  return Math.max(minKwSize, word.fq * maxKwSize);
}

export function getWordW(word, stages) {
