...
var stageH = 500;
var service;
const image_url = '/assets/images/searchpoint/';
var centerClustColor = [212, 0, 90];	// light gray
let cookie;

function getSource() {
  return $('#source option:selected').val();
}

function getPublicationType() {
  return $('#publication-type option:selected').val();
}
...
var DataService = function () {
  ...
  var that = {
    fetchData: function (clustering) {
      ...
      var query = $('#q').val();
      let publicationType = getPublicationType();
      ...
      $.ajax({
        type: "GET",
        url: "/api/v1/searchpoint/query",
        data: {
          q: {keyword: query, type: publicationType, date: {start, end}},
          c: clusteringKey, n: nresults, source, cookie
        },
        dataType: "json",
        async: true,
            ...
            if (clusters != null && clusters.length > 0) {
              ...
              if (getClusteringKey() == 'kmeans') {
                ...
                for (var clustIdx = 0; clustIdx < nClusts; clustIdx++) {
                  ...
                  createWc(cluster.kwords, cluster.x, cluster.y, undefined, stages);
                }
                ...
                createWc(centerClust.kwords, centerClust.x, centerClust.y, 1.3, stages);
              } else {
      ...
      window.history.pushState({
        q: query,
        c: clusteringKey
      }, "", '?q=' + query + '&c=' + clusteringKey + (nresults != 200 ? '&n=' + nresults : ''));
    },
    ...
    fetchItems: function (page) {
      ...
      $.ajax({
        type: "GET",
        url: "/api/v1/searchpoint/rank",
        data: {pos: positions, p: page, source, cookie},
        dataType: "json",
        async: true,
    ...
    fetchKeywords: function (x, y, callback) {
      ...
      $.ajax({
        type: "GET",
        url: "/api/v1/searchpoint/keywords",
        data: {x: serverPos.x, y: serverPos.y, qid: queryId, source},
        dataType: "json",
        async: true,
        success: function (data, textStatus, jqXHR) {
          fetchingKws = false;
...
//				hideTooltip();
  ...
  img.onload = function () {
    ...
    cloudImg.src = image_url + "sugg.png";
...
function Stage(options) {
  var that = {
    ...
    initButtons: function () {
      ...
      backImg.src = image_url + "back.png";
      forwImg.src = image_url + "forward.png";
      centerImg.src = image_url + "target_big.png";
    ...
    initLogo: function () {
      ...
      logo.src = image_url + "logo_small.jpg";
...
function processSubmit(clustering) {
  ...
  //url='/result.html?q=' + $('#q').val() + '&c=' + clustering;
  ...
  var that = {
    ...
    drawPageLink: function (page) {
      if (page == that.currPage + 1)
        ...
        return "<td><a id='link-page-" + (page - 1) + "' class='link-page'>" + page + "</a></td>";
    },
    ...
    drawPageLinks: function () {
      ...
      if (nItems > 0) {
        ...
        if (that.currPage > 0)
          navigHTML += "<td><a id='go-prev'>&lt;&lt;</a></td>"
        ...
        if (that.currPage < nPages - 1)
          navigHTML += "<td><a id='go-next'>&gt;&gt;</a></td>"

        navigHTML += "</tr></table>";
        navig.innerHTML = navigHTML;

        $('#go-prev').on('click', () => service.prevPage());
        $('#go-next').on('click', () => service.nextPage());
        $('.link-page').on('click', function () {
          let id = $(this).attr('id');
          id = id.slice('link-page-'.length);
          jump(+id);
        });
      }
    },
    ...
    drawItems: function () {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (item.title) {
          itemsHTML += "<a class='link' target='_blank' href='" + item.URL + "'>" + item.title + "</a>";
        } else {
        ...
        if (item.title) {
          ...
          itemsHTML += "<span class='url'>";
          itemsHTML += "<a class='link' target='_blank' href='" + item.URL + "'>" + item.displayURL + "</a>";
          itemsHTML += "</span><br />";
        }
...
function initiateQuery() {
  ...
  service.fetchData(undefined);

}

module.exports = {initiateQuery, service};
