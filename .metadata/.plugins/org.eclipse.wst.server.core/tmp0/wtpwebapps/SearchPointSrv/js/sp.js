...
//                	alert(textStatus);
            ...
            // push the URL
            window.history.pushState({ q: query, c: clusteringKey }, "", 'result.jsp?q=' + query + '&c=' + clusteringKey + (nresults != 200 ? '&n=' + nresults : ''));
        },
        ...
        fetchItems: function (page) {
            ...
            $.ajax({
                type: "GET",
                url: "rank",
                data: { pos: JSON.stringify(positions), p: page, qid: queryID },
                dataType: "json",
                async: true,
                success: function (data, textStatus, jqXHR) {
...
function processSubmit (clustering) {
    ...
    var url;
    if (slashIdx < 0)
        url = path + '/result.jsp?q=' + $('#q').val() + '&c=' + clustering;
    else {
        var basePath = path.substring(0, slashIdx);
        url = basePath + '/result.jsp?q=' + $('#q').val() + '&c=' + clustering;
    }
